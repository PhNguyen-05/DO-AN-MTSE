...
const dictionarySamples = {
  negotiate: {
    ...
    meaningEn: "to discuss something formally in order to reach an agreement",
    exampleEn: "The two sides are still negotiating the terms of the agreement.",
    example: "Hai bên vẫn đang đàm phán các điều khoản của thỏa thuận.",
    found: true,
  },
  invoice: {
    word: "Invoice", phonetic: "/ˈɪn.vɔɪs/", type: "Noun",
    meaning: "Hóa đơn",
    meaningEn: "a document listing goods or services provided and their cost",
    exampleEn: "Please send the invoice to the accounting department.",
    example: "Vui lòng gửi hóa đơn đến phòng kế toán.",
    found: true,
  },
  expand: {
    word: "Expand", phonetic: "/ɪkˈspænd/", type: "Verb",
    meaning: "Mở rộng, phát triển",
    meaningEn: "to increase in size, number, or importance",
    exampleEn: "The company plans to expand into overseas markets next year.",
    example: "Công ty có kế hoạch mở rộng sang thị trường nước ngoài vào năm tới.",
    found: true,
  },
  significant: {
    word: "Significant", phonetic: "/sɪɡˈnɪf.ɪ.kənt/", type: "Adjective",
    meaning: "Quan trọng, đáng kể",
    meaningEn: "important or large enough to be noticed or have an effect",
    exampleEn: "There was a significant improvement in sales this quarter.",
    example: "Có sự cải thiện đáng kể trong doanh số quý này.",
    found: true,
  },
  implement: {
    word: "Implement", phonetic: "/ˈɪm.plə.ment/", type: "Verb",
    meaning: "Thực hiện, thi hành",
    meaningEn: "to put a plan or system into operation",
    exampleEn: "We need to implement the new policy starting this quarter.",
    example: "Chúng ta cần thực hiện chính sách mới bắt đầu từ quý này.",
    found: true,
  },
};
              ...
              {userCollections.map((col) => (
                <label
                  ...
                  style={{
                    ...
                    borderRadius: 8, cursor: "pointer",
                    background: selectedId === col.id ? "#e9f0ff" : "#fff", transition: "all 0.15s",
                  }}
                >
                  <input type="radio" name="collection" value={col.id} checked={selectedId === col.id}
                    onChange={() => setSelectedId(col.id)} style={{ accentColor: "#0b57c5" }} />
                  ...
                  <div>
                    ...
                    {col.description && (
                      <p className="vocab-muted" style={{ fontSize: "0.82rem", margin: 0 }}>{col.description}</p>
                    )}
                  </div>
                </label>
              ))}
...
const VocabularyTranslate = ({ onSaveToNotebook, userCollections = [] }) => {
  ...
  const handleConfirmSave = async (targetCollectionId) => {
    ...
    try {
      await callAPI("POST", "/api/vocabulary/notebook", {
        ...
        // lưu ví dụ tiếng Anh vào field example (backend UserVocabulary)
        example:  translatedResult.exampleEn || translatedResult.example || "",
  ...
  // Ví dụ tiếng Anh ưu tiên exampleEn, fallback sang example
  const exampleEn   = translatedResult?.exampleEn || translatedResult?.example || "";
  // Dịch nghĩa tiếng Việt
  const meaningVi   = translatedResult?.meaning || "";
  // Định nghĩa gốc tiếng Anh (nếu có)
  const meaningEn   = translatedResult?.meaningEn || "";
    ...
    <section className="vocab-translate-layout">
      ...
      <article className="vocab-panel">
        <div className="learning-card-head" style={{ marginBottom: 18 }}>
          <div>
            <h2 className="exam-title" style={{ fontSize: "1.25rem" }}>Phiên dịch từ vựng</h2>
            <p className="vocab-muted">
              Tra nghĩa, loại từ, phiên âm và ví dụ trước khi lưu vào sổ tay.
            </p>
          </div>
          <span className="vocab-icon"><i className="bi bi-translate" /></span>
        </div>
        ...
        <form className="vocab-search-form" onSubmit={handleTranslate}>
          ...
          >
            {isTranslating ? (
              ...
              <><i className="bi bi-search" /> Tra từ</>
            )}
          </button>
        </form>
        ...
        <div className="exam-explanation" style={{ marginTop: 20 }}>
          <strong><i className="bi bi-info-circle" /> Cách hoạt động</strong>
          <p style={{ margin: "8px 0 0", fontSize: "0.88rem" }}>
            Tra từ qua từ điển tiếng Anh tự động. Ví dụ hiển thị bằng tiếng Anh để dễ học.
            Nhấn <strong>"Thêm vào sổ tay"</strong> để chọn bộ từ và lưu lại.
          </p>
        </div>
      </article>
      ...
      <article className="vocab-panel">
        {!translatedResult && (
          <div className="learning-empty">
            <span className="vocab-icon study"><i className="bi bi-search-heart" /></span>
        ...
        {translatedResult && translatedResult.found !== false && (
          <div className="vocab-result">
            ...
            {/* Định nghĩa tiếng Anh (nếu có) */}
            {meaningEn && (
              <p style={{ fontSize: "0.88rem", color: "#475569", marginTop: 4, fontStyle: "italic" }}>
                {meaningEn}
              </p>
            )}

            {/* Ví dụ tiếng Anh — highlight từ đang tra */}
            {exampleEn && (
              <div
                style={{
                  marginTop: 16,
              ...
              >
                <p style={{
                  ...
                  <i className="bi bi-chat-quote" /> Ví dụ tiếng Anh
                </p>
                <p style={{ margin: 0, fontStyle: "italic", color: "#334155", lineHeight: 1.6 }}>
                  "{exampleEn}"
                </p>
              </div>
            )}
            ...
            {/* Synonyms nếu có */}
            {translatedResult.synonyms?.length > 0 && (
              <div style={{ marginTop: 12 }}>
                <p style={{ fontSize: "0.78rem", color: "#64748b", fontWeight: 700, marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.06em" }}>
                  Từ đồng nghĩa
                </p>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {translatedResult.synonyms.map((s) => (
                    <button
                      key={s}
                      className="learning-btn"
                      style={{ padding: "3px 10px", fontSize: "0.82rem" }}
                      onClick={() => { setSearchWord(s); setSaveMessage(""); }}
                    >
                      {s}
                    </button>
                  ))}
            ...
            >
              {isSaving ? (
                ...
                <><i className="bi bi-plus-circle" /> Thêm từ vào sổ tay</>
              )}
            </button>
          </div>
        )}
