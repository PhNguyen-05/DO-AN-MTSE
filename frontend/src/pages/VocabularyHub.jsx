...
// FIX: đọc collectionId từ backend thay vì hardcode null
...
//   status:       w.status     || "Đang học",
//   lastReviewed: w.lastReviewed || w.updatedAt || new Date().toISOString(),
//   collectionId: w.collectionId || null,   // ✅ đọc từ backend
//   isUserAdded:  true,
// });
...
//               src={resolvedImageUrl}
//               alt={`Ảnh minh họa từ "${word.word}"`}
//               style={{ width: "100%", maxHeight: "76vh", objectFit: "contain", borderRadius: 12, background: "#fff" }}
//             />
//             <button
//               onClick={() => setShowImage(false)}
//               style={{
...
//       const savedId = saved?.vocab?._id || saved?._id || `local_${Date.now()}`;
...
//     } catch (err) {
//       if (err.status === 409) {
//         // Từ đã tồn tại → báo warning, không chuyển tab
//         showToast(`Từ "${newWord.word}" đã có trong sổ tay.`, "warning");
//         return;
//       }

//       // Lỗi khác (network, chưa đăng nhập...) → lưu local tạm
//       const newVocab = {
//         ...newWord,
//         id:           `local_${Date.now()}`,
//         collectionId: targetCollectionId || null,
...
//       setVocabularies((prev) => [newVocab, ...prev]);
//       showToast(
//         `Đã lưu "${newWord.word}" cục bộ${colName ? ` vào bộ "${colName}"` : ""}.`,
//         "warning"
//       );
//       setActiveTab("list");
//       setStudyMode(null);
