...
const checkExamAccess = async (userId, examId) => {
  ...
  // User Premium có quyền truy cập tất cả đề thi
  if (isPremium) {
    return { allowed: true, exam };
  }

  // Kiểm tra Purchase bundle
  ...
  // Kiểm tra Purchase riêng listening/reading
  const listeningPurchase = await Purchase.findOne({
    user: userId,
    exam: examId,
    packageType: "listening",
    status: "paid",
  }).lean();

  const readingPurchase = await Purchase.findOne({
    user: userId,
    exam: examId,
    packageType: "reading",
    status: "paid",
  ...
  // Nếu có mua ít nhất một phần thì cho phép truy cập
  if (listeningPurchase || readingPurchase) {
    return { allowed: true, exam };
  }

  return { allowed: false, exam };
};
...
const getAccessibleVocabSets = async (req, res, next) => {
  try {
    ...
    const result = allSets.map((set) => {
      ...
      const canAccess = isFree || hasPurchased || (isPremium && set.accessType === "premium");
